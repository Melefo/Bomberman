...
namespace RayLib
{
    ...
    {
        for (int i = 0; i < _animsCount; i++) {
            ::UnloadModelAnimation(_modelAnimations.at(i));
        }
    }
    ...
    {
        _frame++;
        ::UpdateModelAnimation(model.GetModel(), _modelAnimations.at(animationIndex), _frame);
        if (_frame >= _modelAnimations.at(0).frameCount)
            _frame = 0;
    }
}
